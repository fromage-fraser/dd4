...
- With distro installed, install `make`, `docker`
...
* If the destroyed `ACT_OBJECT` mob is undead, its generated remains receive +
`ITEM_UNDEAD`. Ordinary NPC/PC corpses and generated body parts from undead +
characters receive the same provenance flag.
...
of treasure in hoards (particularly money) is a good idea.  Random hoard +
generation has been added to the auto quest function (roughly a third of +
automated quests will be of this type), and random loot can be found in these +
hoards. +
