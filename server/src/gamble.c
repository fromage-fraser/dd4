 ...
 */
    ...
    {
	send_to_char( "What do you wish to bet: {WUpper{x, {WLower{x, or {WTriple{x?\n\r",
		     ch );
	return;
    }
/*
 ...
 */
    ...
    sprintf( msg, "$n rolls the dice: they come up {G%d{x, {G%d{x, and {G%d{x",
	    die1, die2, die3 );
	...
	if ( ichoice == 3 )
	{
	    ...
	    act( "It's a {WTRIPLE{x!  You win {Y$t gold coins{x!",
		ch, haul, NULL, TO_CHAR );
	    ch->gold += amount;
	    pGameIndex->bankroll -= amount;
	}
	else
	    send_to_char( "It's a {RTRIPLE{x!  You lose!\n\r", ch );

	return;
    }

    sprintf( buf, ", totalling {G%d{x.", total );
    strcat( msg, buf );
    act( msg, croupier, NULL, ch, TO_VICT );
	...
	amount *= 2;
	sprintf( haul, "%d", amount );
	act( "You win {Y$t gold coins{x!", ch, haul, NULL, TO_CHAR );
	ch->gold += amount;
	pGameIndex->bankroll -= amount;
    }
    else
	send_to_char( "{WSorry, better luck next time!{x\n\r", ch );
 ...
 */
	...
	case 1:
	   ...
	   break;
	case 2:
	   strcpy ( msg, "Let's see those dice!" );
	   break;
	case 3:
	   strcpy ( msg, "You'd better let me win!" );
	   break;
	case 4:
	   strcpy ( msg, "Here's my cash--now roll'em!" );
	   break;
	case 5:
    ...
    sprintf( msg, "$n rolls your dice: they come up {G%d{x and {G%d{x",
            die1, die2 );

    sprintf( buf, ", totalling {G%d{x.", total1 );
    strcat( msg, buf );
    act( msg, croupier, NULL, ch, TO_VICT );

    sprintf( msg, "$n rolls his dice: they come up {G%d{x and {G%d{x",
            die3, die4 );

    sprintf( buf, ", totalling {G%d{x.", total2 );
    strcat( msg, buf );
    act( msg, croupier, NULL, ch, TO_VICT );
	...
	int amount, int cheat, char *argument )
{
    ...
    {
        send_to_char( "What do you wish to bet: {WUnder{x, {WOver{x, or {WSeven{x?\n\r", ch );
        return;
    }
 ...
 */
    ...
    if( total == 7 )
    {
        ...
        if ( ichoice == 3 )
        {
            ...
            amount *= 5;
            sprintf( haul, "%d", amount );
            act( "It's a {WSEVEN{x!  You win {Y$t gold coins{x!",
                ch, haul, NULL, TO_CHAR );
            ch->gold += amount;
	    pGameIndex->bankroll -= amount;
        }
        else
            send_to_char( "It's a {RSEVEN{x!  You lose!\n\r", ch );
