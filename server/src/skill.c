...
void do_imbue (CHAR_DATA *ch, char *argument)
{
       ...
       modifier = imbue_list[random_buff].apply_buff;
        ...
        if (ch->pcdata->learned[gsn_imbue] > 75)
        {
                ...
                paf->duration       = -1;
                paf->location       = modifier;
                paf->modifier       = imbue_list[random_buff2].base_gain * ch->pcdata->learned[gsn_imbue] / 5;
                paf->bitvector      = 0;
                paf->next           = obj->affected;
                obj->affected       = paf;
        ...
        if (ch->pcdata->learned[gsn_imbue] > 97)
        {
                ...
                paf->duration       = -1;
                paf->location       = modifier;
                paf->modifier       = imbue_list[random_buff3].base_gain * ch->pcdata->learned[gsn_imbue] / 5;
                paf->next           = obj->affected;
                obj->affected       = paf;
                random_buff = -1;
        }
