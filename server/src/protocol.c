...
void ProtocolDestroy(protocol_t *apProtocol)
{
   ...
   if (apProtocol->MediaAreaName)
   {
      free_string(apProtocol->MediaAreaName);
      apProtocol->MediaAreaName = NULL;
   }
   if (apProtocol->MediaRoomName)
   {
      free_string(apProtocol->MediaRoomName);
      apProtocol->MediaRoomName = NULL;
   }
   if (apProtocol->MediaSectorName)
   {
...
static void SendATCP(descriptor_t *apDescriptor, const char *apVariable, const char *apValue)
{
   ...
   if (apVariable != NULL && apValue != NULL)
   {
      ...
      if (RequiredBuffer >= MAX_VARIABLE_LENGTH)
      {
         log_string("RequiredBuffer too long");
         if (RequiredBuffer - strlen(apValue) < MAX_VARIABLE_LENGTH)
         {
            sprintf(ATCPBuffer,
...
const struct gmcp_package_struct GMCPPackageTable[GMCP_PACKAGE_MAX + 1] =
    {
        ...
        {GMCP_QUEST, GMCP_SUPPORT_CHAR, "Char", "Quest"},
...
const struct gmcp_variable_struct GMCPVariableTable[GMCP_MAX + 1] =
    {
        ...
        {GMCP_HUNGER, GMCP_VITALS, "hunger", GMCP_NUMBER},
        {GMCP_MAX_HUNGER, GMCP_VITALS, "maxhunger", GMCP_NUMBER},
        {GMCP_THIRST, GMCP_VITALS, "thirst", GMCP_NUMBER},
        {GMCP_MAX_THIRST, GMCP_VITALS, "maxthirst", GMCP_NUMBER},
        {GMCP_DRUNK, GMCP_VITALS, "drunk", GMCP_NUMBER},
        {GMCP_MAX_DRUNK, GMCP_VITALS, "maxdrunk", GMCP_NUMBER},
        ...
        {GMCP_QUEST_TYPE, GMCP_QUEST, "type", GMCP_STRING},
        {GMCP_QUEST_COMPLETE, GMCP_QUEST, "complete", GMCP_NUMBER},
        {GMCP_QUEST_COUNTDOWN, GMCP_QUEST, "countdown", GMCP_NUMBER},
        {GMCP_QUEST_NEXT, GMCP_QUEST, "nextquest", GMCP_NUMBER},
        {GMCP_QUEST_POINTS, GMCP_QUEST, "points", GMCP_NUMBER},
        {GMCP_QUEST_TOTAL_POINTS, GMCP_QUEST, "total_points", GMCP_NUMBER},
        {GMCP_QUEST_MOB, GMCP_QUEST, "mob_vnum", GMCP_NUMBER},
        {GMCP_QUEST_OBJECT, GMCP_QUEST, "object_vnum", GMCP_NUMBER},
        {GMCP_QUEST_GIVER_VNUM, GMCP_QUEST, "giver_vnum", GMCP_NUMBER},
        {GMCP_QUEST_GIVER_NAME, GMCP_QUEST, "giver_name", GMCP_STRING},
        {GMCP_QUEST_TARGET_NAME, GMCP_QUEST, "target_name", GMCP_STRING},
...
void GMCP_Media_Default_Ensure(descriptor_t *d, const char *base_url)
{
   ...
   if (!d->pProtocol->bGMCPSupport[GMCP_SUPPORT_CLIENT_MEDIA])
      return;
   if (d->pProtocol->bClientMediaDefaultSent)
      return;
