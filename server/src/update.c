...
void advance_level(CHAR_DATA *ch)
{
        ...
        tmp = ((double)get_curr_wis(ch) + (double)get_curr_str(ch) + (double)get_curr_dex(ch)) / 3;
        ...
        if (ch->sub_class == SUB_CLASS_MONK || ch->sub_class == SUB_CLASS_BARD)
        {
                add_int_prac++;
        }
        if (tmp > 15)
...
void gain_exp(CHAR_DATA *ch, int gain)
{
        ...
        while (ch->level < LEVEL_HERO && ch->exp >= (level_table[ch->level].exp_total))
        {
                ...
                if (ch->level == 75 || ch->level == 85 || ch->level == 95 || ch->level == LEVEL_HERO)
                {
...
int hit_gain(CHAR_DATA *ch)
{
        ...
        if ((ch->sub_class == SUB_CLASS_VAMPIRE && ch->rage < ch->max_rage / 10) || (ch->sub_class == SUB_CLASS_WEREWOLF && ch->rage > 90))
                gain -= gain / 3;
        ...
        if (ch->sub_class == SUB_CLASS_WITCH && (ch->in_room->sector_type == SECT_SWAMP || ch->in_room->sector_type == SECT_FOREST))
        {
        ...
        if ((ch->race == RACE_HUMAN && ch->in_room->sector_type == SECT_CITY) || (ch->race == RACE_FAE && ch->in_room->sector_type == SECT_AIR) || (ch->race == RACE_WILD_ELF && ch->in_room->sector_type == SECT_FOREST) || (ch->race == RACE_SATYR && ch->in_room->sector_type == SECT_FOREST) || (ch->race == RACE_HALF_DRAGON && ch->in_room->sector_type == SECT_MOUNTAIN) || (ch->race == RACE_DWARF && ch->in_room->sector_type == SECT_MOUNTAIN) || (ch->race == RACE_OGRE && ch->in_room->sector_type == SECT_HILLS) || (ch->race == RACE_HALFLING && ch->in_room->sector_type == SECT_FIELD) || (ch->race == RACE_DWARF && ch->in_room->sector_type == SECT_MOUNTAIN) || (ch->race == RACE_TROLL && ch->in_room->sector_type == SECT_SWAMP) || (ch->race == RACE_YUAN_TI && ch->in_room->sector_type == SECT_DESERT) || (ch->race == RACE_DUERGAR && ch->in_room->sector_type == SECT_MOUNTAIN))
        {
        ...
        if ((ch->race == RACE_SAHUAGIN || ch->race == RACE_GRUNG) && ((ch->in_room->sector_type == SECT_UNDERWATER) || (ch->in_room->sector_type == SECT_UNDERWATER_GROUND) || (ch->in_room->sector_type == SECT_WATER_SWIM) || (ch->in_room->sector_type == SECT_SWAMP) || (ch->in_room->sector_type == SECT_WATER_NOSWIM)))
        {
        ...
        if (!IS_NPC(ch) && !ch->fighting && is_affected(ch, gsn_coil))
        {
...
int mana_gain(CHAR_DATA *ch)
{
        ...
        if ((ch->class == CLASS_SHAPE_SHIFTER) && (ch->sub_class == SUB_CLASS_NONE))
                gain *= 2;
...
int rage_gain(CHAR_DATA *ch)
{
        ...
        if (IS_NPC(ch) || !(ch->sub_class == SUB_CLASS_VAMPIRE || ch->sub_class == SUB_CLASS_WEREWOLF) || IS_AFFECTED(ch, AFF_NON_CORPOREAL))
                return 0;
...
void mobile_update(void)
{
        ...
        for (ch = char_list; ch; ch = ch->next)
        {
                ...
                if (!IS_NPC(ch) || !ch->in_room || ch->mount || ch->rider || IS_AFFECTED(ch, AFF_CHARM))
                        continue;
                ...
                if (IS_NPC(ch))
                {
                        if (IS_SET(ch->act, ACT_DIE_IF_MASTER_GONE) && (!ch->master || ch->master->in_room != ch->in_room) && !ch->fighting)
                        {
                        ...
                        if (IS_AFFECTED(ch, AFF_SWALLOWED) && (!ch->inside || ch->inside->in_room != ch->in_room))
                        {
                                strip_swallow(ch);
                                continue;
                        }
                        ...
                        {
                                if (tournament_status == TOURNAMENT_STATUS_NONE || tournament_status == TOURNAMENT_STATUS_FINISHED || !IS_SET(ch->in_room->room_flags, ROOM_PLAYER_KILLER))
                                {
                                        if (is_still_alive_in_tournament(ch) && tournament_status == TOURNAMENT_STATUS_RUNNING)
                                        {
                ...
                if (IS_SET(ch->act, ACT_SCAVENGER) && ch->in_room->contents && !number_bits(2))
                {
                        OBJ_DATA *obj;
                 ...
                 * Flee
                 */
                if (ch->hit < (ch->max_hit / 2) && !IS_SET(ch->act, ACT_SENTINEL) && (door = number_bits(3)) <= 5 && (pexit = ch->in_room->exit[door]) && !IS_SET(ch->act, ACT_DIE_IF_MASTER_GONE) && (!IS_SET(ch->act, ACT_STAY_AREA) || pexit->to_room->area == ch->in_room->area) && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED) && !IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB))
                {
                        move_char(ch, door);
                }
        }
...
void weather_update()
{
        ...
        case SKY_CLOUDLESS:
                if (weather_info.mmhg < 990 || (weather_info.mmhg < 1010 && !number_bits(2)))
                {
        ...
        case SKY_CLOUDY:
                if (weather_info.mmhg < 970 || (weather_info.mmhg < 990 && !number_bits(2)))
                {
        ...
        case SKY_RAINING:
                ...
                if (weather_info.mmhg > 1030 || (weather_info.mmhg > 1010 && !number_bits(2)))
                {
        ...
        case SKY_LIGHTNING:
                if (weather_info.mmhg > 1010 || (weather_info.mmhg > 990 && !number_bits(2)))
                {
...
void char_update(void)
{
        ...
        for (ch = char_list; ch; ch = ch->next)
        {
                ...
                if (!IS_NPC(ch) && (ch->level < LEVEL_IMMORTAL || (!ch->desc && !IS_SWITCHED(ch))))
                {
                        ...
                        {
                                if (!IS_NPC(gch) && is_same_group(gch, ch) && is_affected(gch, gsn_song_of_sustenance))
                                {
                ...
                {
                        if ((IS_AFFECTED(ch, AFF_CONFUSION)) && (!is_affected(ch, gsn_confusion)))
                        {
                        ...
                        */
                        ...
                        if (IS_AFFECTED(ch, AFF_CONFUSION) && (door = number_bits(5)) <= 5 && (pexit = ch->in_room->exit[door]) && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED))
                        {
                                if (ch->position == POS_SLEEPING)
                                {
                        ...
                        if ((is_affected(ch, gsn_poison) && IS_AFFECTED(ch, AFF_POISON)) || IS_AFFECTED(ch, AFF_PRAYER_PLAGUE))
                        {
                                if (IS_AFFECTED(ch, AFF_MEDITATE))
                                        REMOVE_BIT(ch->affected_by, AFF_MEDITATE);
                        ...
                        /* Drowning? */
                        if (!IS_NPC(ch) && ch->level <= LEVEL_HERO && ch->position > POS_INCAP && ch->form != FORM_SNAKE && ch->form != FORM_HYDRA && ch->form != FORM_DEMON && ch->race != RACE_SAHUAGIN && ch->race != RACE_GRUNG && ch->sub_class != SUB_CLASS_VAMPIRE && !is_affected(ch, gsn_mist_walk) && !is_affected(ch, gsn_astral_sidestep) && ((ch->in_room->sector_type == SECT_UNDERWATER) || (ch->in_room->sector_type == SECT_UNDERWATER_GROUND)) && !is_affected(ch, gsn_breathe_water))
                        {
...
void obj_update()
{
        ...
        for (obj = object_list; obj; obj = obj_next)
        {
                ...
                {
                        if (obj->item_type == ITEM_WEAPON && IS_OBJ_STAT(obj, ITEM_POISONED))
                        {
                                act("Poison is beginning to seriously damage $p.",
                                    obj->carried_by, obj, NULL, TO_CHAR);
                        }

                        else if (obj->item_type == ITEM_WEAPON && (IS_OBJ_STAT(obj, ITEM_SHARP) || IS_OBJ_STAT(obj, ITEM_BLADE_THIRST)))
                        {
                                act("$p is beginning to deteriorate.",
                                    obj->carried_by, obj, NULL, TO_CHAR);
                        }

                        else if (obj->item_type == ITEM_ARMOR && IS_OBJ_STAT(obj, ITEM_FORGED))
                        {
...
void aggr_update()
{
        ...
        for (d = descriptor_list; d; d = d->next)
        {
                 ...
                 */
                if (++bloodlust_count > 19 && has_ego_item_effect(ch, EGO_ITEM_BLOODLUST) && IS_AWAKE(ch) && !ch->fighting && !(IS_SET(ch->in_room->room_flags, ROOM_SAFE) || IS_SET(ch->in_room->area->area_flags, AREA_FLAG_SAFE)) && !IS_AFFECTED(ch, AFF_NON_CORPOREAL) && (ch->level > LEVEL_HERO || !ch->wait))
                {
                        bloodlust_count = 0;

                        for (mch = ch->in_room->people; mch; mch = mch->next_in_room)
                        {
                                if (IS_NPC(mch) && !mch->rider && !IS_SET(mch->act, ACT_CLAN_GUARD) && !number_bits(2))
                                {
                ...
                for (mch = ch->in_room->people; mch; mch = mch->next_in_room)
                {
                        ...
                        if ((IS_AFFECTED(mch, AFF_DETECT_CURSE) && is_cursed(ch)) && (mch != ch) && (IS_NPC(mch) && !IS_NPC(ch)) && (ch->level < LEVEL_IMMORTAL))
                        {
                                cursed_utterance = 1;
                                goto fightloop;
                        }

                        if (!IS_NPC(mch) || !IS_SET(mch->act, ACT_AGGRESSIVE) || !can_see(mch, ch) || !IS_AWAKE(mch) || mch->fighting || (IS_AFFECTED(ch, AFF_DETER) && !IS_SET(mch->act, ACT_CLAN_GUARD)) || IS_SET(mch->in_room->room_flags, ROOM_SAFE) || IS_SET(mch->in_room->area->area_flags, AREA_FLAG_SAFE) || IS_AFFECTED(mch, AFF_CHARM) || IS_AFFECTED(ch, AFF_NON_CORPOREAL) || (IS_SET(mch->act, ACT_WIMPY) && IS_AWAKE(ch)) || has_tranquility(ch) || ch->level > LEVEL_HERO || mch->rider || mch->deleted || (ch->level > mch->level + 10))
                                continue;

                fightloop:

                        /* If in a clan headquarters, don't attack anyone */
                        if (mch->in_room->area->low_level == -4 && mch->in_room->area->high_level == -4)
                                continue;
                        ...
                        if (CAN_SPEAK(mch) && (cursed_utterance == 1) && (mch->hit == mch->max_hit))
                        {
                                cursed_utterance = 0;
                                tmp = number_percent();
                                if (tmp < 25)
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
        {
                if (d->character && d->connected == CON_PLAYING && !IS_NPC(d->character))
                {
...
#else
                                                if (buf[0] == '\0')
                                                        sprintf(buf, "[ {{ \"name\": \"%s\", \"level\": \"%d\", \"hp\": \"%d\", \"maxhp\": \"%d\" }", enemy->name, enemy->level, enemy->hit, enemy->max_hit);
                                                else
                                                {
                        ...
                        for (iShow = 0; iShow < nShow; iShow++)
                        {
                                ...
                                if (!IS_SET(d->character->act, PLR_COMBINE) || prgnShow[iShow] == 1)
                                {
                                        if (buf[0] == '\0')
                                        {
                                                colourconv_8bit(buf3, prgpstrShow[iShow], d->character);
                ...
                /* Notify web clients of GMCP updates */
                webgate_notify_gmcp_update(d);
        }
        return;
}
