...
int translate_telopts(DESCRIPTOR_DATA *d, unsigned char *src, int srclen, unsigned char *out, int outlen)
{
	...
	push_call("translate_telopts(%p,%p,%d,%p,%d)",d,src,srclen,out,outlen);
	...
	*pto = 0;
	...
	if (IS_SET(d->mth->comm_flags, COMM_FLAG_REMOTEECHO))
	{
		...
		for (cnt = 0 ; cnt < skip ; cnt++)
		{
			switch (out[cnt])
			{
				...
				default:
					if (IS_SET(d->mth->comm_flags, COMM_FLAG_PASSWORD))
					{
...
{
	REMOVE_BIT(d->mth->comm_flags, COMM_FLAG_PASSWORD);
...
void send_eor( DESCRIPTOR_DATA *d )
{
	if (IS_SET(d->mth->comm_flags, COMM_FLAG_EOR))
	{
			...
			case IAC:
				*pto = 0;
				...
				else
				{
					...
					{
						if (IS_SET(d->mth->mtts, MTTS_FLAG_256COLORS))
						{
							SET_BIT(d->mth->comm_flags, COMM_FLAG_256COLORS);
						}

						if (IS_SET(d->mth->mtts, MTTS_FLAG_UTF8))
						{
	...
	while (i < srclen && src[i] != SE && src[i] != src[4])
	{
		...
		*pto = 0;
		...
		else if (src[3] == CHARSET_REJECTED)
		{
			if (!strcasecmp(val, "UTF-8"))
			{
				REMOVE_BIT(d->mth->comm_flags, COMM_FLAG_UTF8);
			}
		}
		i++;
	}
