...
void do_wizhelp(CHAR_DATA *ch, char *argument)
{
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
                ...
                if (rch->pcdata->learned[sn] >= 100 && wiz_do(ch, skill_table[sn].name) && skill_table[sn].prac_type == TYPE_WIZ)
                {
...
void do_ostat(CHAR_DATA *ch, char *argument)
{
        ...
        if (obj->extra_flags)
        {
                ...
                if ((has_bp_flag) && (obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_LIGHT))
                {
...
void do_tfind(CHAR_DATA *ch, char *argument)
{
        ...
        for (vnum = 0; nMatch < top_mob_index; vnum++)
        {
                if ((pMobIndex = get_mob_index(vnum)))
                {
                        ...
                        if (fAll || (IS_SET(pMobIndex->act, ACT_PRACTICE) && (pMobIndex->skills)))
                        {
...
void do_mwhere(CHAR_DATA *ch, char *argument)
{
        ...
        {
                if (IS_NPC(victim) && victim->in_room && multi_keyword_match(arg, victim->name))
                {
                        found = TRUE;
...
void do_mnstat(CHAR_DATA *ch, char *argument)
{
        ...
        if (pMobIndex->short_descr[0] != '\0' && pMobIndex->long_descr[0] != '\0')
        {
                sprintf(buf, "Short description: {W%s{x\n\rMobspec: {W%s{x\n\rLong description: \n\r  {W%s{x",
                        pMobIndex->short_descr,
                        pMobIndex->mobspec,
...
void do_promote(CHAR_DATA *ch, char *argument)
{
        ...
        if (level <= victim->clan_level)
        {
                ...
                sprintf(buf, "You are DEMOTED to %s\n\r",
                        victim->clan_level ? clan_title[victim->clan][victim->clan_level]
                                           : "level zero");
                send_to_char(buf, victim);
        }
        else
...
void do_mset(CHAR_DATA *ch, char *argument)
{
        ...
        if (!str_cmp(arg2, "str"))
        {
                ...
                max = 21 + race_table[ch->race].str_bonus + class_table[ch->class].class_stats[0];
        ...
        if (!str_cmp(arg2, "int"))
        {
                ...
                max = 21 + race_table[ch->race].int_bonus + class_table[ch->class].class_stats[1];
        ...
        if (!str_cmp(arg2, "wis"))
        {
                ...
                max = 21 + race_table[ch->race].wis_bonus + class_table[ch->class].class_stats[2];
        ...
        if (!str_cmp(arg2, "dex"))
        {
                ...
                max = 21 + race_table[ch->race].dex_bonus + class_table[ch->class].class_stats[3];
        ...
        if (!str_cmp(arg2, "con"))
        {
                ...
                max = 21 + race_table[ch->race].con_bonus + class_table[ch->class].class_stats[4];
        ...
        if (!str_cmp(arg2, "age"))
        {
                if ((value < MIN_MSET_AGE) || (value > MAX_MSET_AGE))
                {
        ...
        if (!str_cmp(arg2, "thirst"))
        {
                ...
                if ((ivalue < 0 || ivalue > MAX_DRINK) && get_trust(victim) < LEVEL_IMMORTAL)
                {
                        sprintf(buf, "Thirst range is 0 to %d.\n\r", MAX_DRINK);
                        send_to_char(buf, ch);
                        return;
                }
                else if (ivalue < -1 || ivalue > MAX_DRINK)
                {
        ...
        if (!str_cmp(arg2, "full"))
        {
                ...
                if ((ivalue < 0 || ivalue > MAX_FOOD) && get_trust(victim) < LEVEL_IMMORTAL)
                {
...
void do_rename(CHAR_DATA *ch, char *argument)
{
        ...
        if ((obj->item_type == ITEM_CLAN_OBJECT || !is_obj_owner(obj, ch)) && !IS_IMMORTAL(ch))
        {
...
void do_force(CHAR_DATA *ch, char *argument)
{
        ...
        {
                if (argument[0] == cmd_table[cmd].name[0] && !str_prefix(argument, cmd_table[cmd].name) && (cmd_table[cmd].level > trust))
                {
...
void do_sstime(CHAR_DATA *ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || arg2[0] == '\0' || (strlen(arg2) != 8 && (arg2[0] != '*' && strlen(arg2) != 1)))
        {
...
void other_prac(CHAR_DATA *ch, CHAR_DATA *victim)
{
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
                learned = !IS_NPC(victim) ? victim->pcdata->learned[sn] : victim->pIndexData->skills->learned[sn];
...
void do_cando(CHAR_DATA *ch, char *argument)
{
        ...
        /* get results */
        if ((!IS_NPC(victim) && CAN_DO(victim, sn)) || (IS_NPC(victim) && victim->pIndexData->skills->learned[sn]))
        {
                sprintf(buf, "\n%s can do %s (%d) at %d%%.\n\r", victim->name,
                        skill_table[sn].name, sn,
                        !IS_NPC(victim) ? victim->pcdata->learned[sn] : victim->pIndexData->skills->learned[sn]);
        }
        else
        {
...
void do_reset(CHAR_DATA *ch, char *argument)
{
        ...
        {
                if (!IS_NPC(pch) && IS_AWAKE(pch) && pch->in_room && pch->in_room->area == area)
                {
...
#define KEY_ROOM "dd.ambient.room"
        ...
        if (ch->desc && ch->desc->pProtocol && ch->desc->pProtocol->bGMCP && (!ch->in_room->ambient_sound || !*ch->in_room->ambient_sound || ch->in_room->ambient_volume <= 0))
        {
                protocol_t *proto = ch->desc->pProtocol;
...
#define KEY_ROOM "dd.ambient.room"
        ...
        if (!str_cmp(arg1, "off"))
        {
                ...
                if (ch->desc && ch->desc->pProtocol && ch->desc->pProtocol->bGMCP)
                {
                        ...
                        if (ch->in_room->area && ch->in_room->area->ambient_sound && *ch->in_room->area->ambient_sound && ch->in_room->area->ambient_volume > 0)
                        {
                                GMCP_Media_Default_Ensure(ch->desc, "https://www.dragons-domain.org/main/gui/custom/audio/");
...
void do_mcmp(CHAR_DATA *ch, char *argument)
{
        ...
        if (!ch->desc->pProtocol || !ch->desc->pProtocol->bGMCPSupport[GMCP_SUPPORT_CLIENT_MEDIA])
        {
...
#define KEY_ROOM "dd.ambient.room"
        ...
        /* ----- PLAY helpers ----- */
        if (!str_cmp(arg1, "area") || !str_cmp(arg1, "room") || !str_cmp(arg1, "music") || !str_cmp(arg1, "test"))
        {
                ...
                if (arg3[0] != '\0')
                        volume = URANGE(1, atoi(arg3), 100);
                else if (argument && *argument)
                        volume = URANGE(1, atoi(argument), 100);
