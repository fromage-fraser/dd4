...
int gsn_heighten;
int gsn_shadow_form;
int gsn_infernal_fury;
int gsn_possession;
int gsn_steal_strength;
int gsn_abyssal_hand;
...
void load_mob_spec(FILE *fp, MOB_INDEX_DATA *pMobIndex);
...
void boot_db(void)
{
        ...
        /*
         * Check secure gameplay randomness before loading the world.
         */
        {
                rng_init();
                log_string("Gameplay RNG: OpenSSL RAND_bytes.");
        }
        ...
        /*
         * Cache prerequisites for skill tree visualization
         */
        {
                cache_prerequisites();
        }
        ...
        for (;;)
        {
                ...
                for (;;)
                {
                        ...
                        if (letter == 'D')
                        {
                                ...
                                case 11:
                                        pexit->exit_info = EX_ISDOOR | EX_PICKPROOF | EX_BASHPROOF | EX_PASSPROOF | EX_SECRET;
                                        break;
...
void reset_area(AREA_DATA *pArea)
{
        ...
        for (pReset = pArea->reset_first; pReset; pReset = pReset->next)
        {
                ...
                case 'O':
                        ...
                        if (((pArea->nplayer > 0) && (pObjIndex->item_type != ITEM_HOARD)) || (count_obj_list(pObjIndex, pRoomIndex->contents) > 0))
                        {
                                last = FALSE;
                                break;
                        }
                ...
                case 'I':
                        ...
                        if (((pArea->nplayer > 0) && (pObjIndex->item_type != ITEM_HOARD)) || (count_obj_list(pObjIndex, pRoomIndex->contents) >= pReset->arg3))
                        {
                                last = FALSE;
                                break;
                        }
                ...
                case 'P':
                        ...
                        if ((pArea->nplayer > 0) || !(obj_to = get_obj_type(pObjToIndex)) || (count_obj_list(pObjIndex, obj_to->contains) > 0))
                        {
                                last = FALSE;
                                break;
                        }
                ...
                case 'E':
                        ...
                        if (pReset->command == 'E')
                        {
                                ...
                                case WEAR_HOLD:
                                        if (obj->item_type == ITEM_WAND || obj->item_type == ITEM_STAFF || obj->item_type == ITEM_POTION || obj->item_type == ITEM_PILL || obj->item_type == ITEM_SCROLL)
                                        {
                                                mob->exp_modifier += 5;
                                        }
                                        break;
...
OBJ_DATA *create_object(OBJ_INDEX_DATA *pObjIndex, int level, char *rank, int randomise)
{
        ...
        */
        ...
        case ITEM_LIGHT:
                if ((obj->how_created <= CREATED_NO_RANDOMISER) || IS_OBJ_STAT(obj, ITEM_DONOT_RANDOMISE))
                {
                ...
                else if ((obj->how_created == CREATED_WEAK_RANDOMISER) || IS_OBJ_STAT(obj, ITEM_WEAK_RANDOMISE))
                {
                        ...
                        for (paf = obj->pIndexData->affected; paf; paf = paf->next)
                        {
                                ...
                                if (waf->location != APPLY_NONE && paf->modifier != 0 && strcmp(affect_loc_name(paf->location), "(unknown)"))
                                {
...
void do_areas(CHAR_DATA *ch, char *argument)
{
        ...
        for (pArea = area_first; pArea; pArea = pArea->next)
        {
                if (!level || (pArea->low_level <= level && (pArea->high_level >= level || pArea->high_level == -1 || pArea->low_level == -2)))
                {
                        /*
        ...
        for (pArea = first_sort; pArea; pArea = pArea->next)
        {
                ...
                if (multi_keyword_match(arg, pArea->name))
                {
                        ...
                        for (victim = char_list; victim; victim = victim->next)
                        {
                                ...
                                {
                                        if (IS_NPC(victim) && victim->in_room && (rank_sn(victim) > 1))
                                        {
                        ...
                        for (obj = object_list; obj; obj = obj->next)
                        {
                                ...
                                else
                                {
                                        sprintf(buf, "[%-3d] {g%s{x in {c%s{x at [%5d] (Obj Score:%4d)\n\r",
                                                ...
                                                (!in_obj->in_room) ? 0 : in_obj->in_room->vnum, calc_item_score(obj));
                                }
