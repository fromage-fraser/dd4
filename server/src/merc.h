...
#define IS_FULL_MOON (weather_info.moonlight == MOON_FULL && weather_info.sunlight == SUN_DARK)
...
#define SET_DEITY_TYPE_TIMER(ch, type, value) \
        ((ch)->pcdata->deity_type_timer[(type)] = URANGE(0, ((ch)->pcdata->deity_type_timer[(type)] + (value)), DEITY_TIMER_MAX))

#define SET_DEITY_PERSONALITY_TIMER(ch, type, value) \
        ((ch)->pcdata->deity_personality_timer[(type)] = URANGE(0, ((ch)->pcdata->deity_personality_timer[(type)] + (value)), DEITY_TIMER_MAX))
...
#define CAN_WEAR(class, cur_form, obj, part, loc) \
        (IS_SET(obj->wear_flags, part) && (IS_NPC(ch) || (loc_wear_table[class].can_wear[loc] && form_wear_table[cur_form].can_wear[loc] && wear_table[class].can_wear[obj->item_type])))
