...
void do_strangle (CHAR_DATA *ch, char *argument) 
{
	...
	{
		send_to_char("You attempt to wrap yourself around your victims neck, but fail.\n\r",ch);
		return; 
	}
...
void do_web (CHAR_DATA *ch, char *argument) 
{
	...
	if (arg[0] == '\0') 
	{
		send_to_char("Who do you want to capture in your web?\n\r", ch);
		return;
	}
...
void do_morph_phoenix (CHAR_DATA *ch, bool to_form) 
{
	...
	if (to_form)
	{
		...
		{   
			...
			send_to_char("A Scintillating globe forms around your form!\n\r", ch);
