...
const struct cmd_type cmd_table[] =
    {
        ...
        {"harvest", do_harvest, POS_STANDING, 0, LOG_NORMAL},
        ...
        {"splash", do_splash, POS_STANDING, 0, LOG_NORMAL},
        ...
        {"skilltree", do_skilltree, POS_SLEEPING, 0, LOG_NORMAL},
        ...
        {"mpotimer", do_mpotimer, POS_DEAD, 0, LOG_NORMAL},
         {"mpcost", do_mpcost, POS_DEAD, 0, LOG_NORMAL},
...
void interpret(CHAR_DATA *ch, char *argument)
{
        ...
        if ((cmd_table[cmd].position == POS_FIGHTING) && (IS_AFFECTED(ch, AFF_PRONE)) && (str_cmp("cast", cmd_table[cmd].name)))
        {
        ...
        if (((!IS_NPC(ch) && IS_SET(ch->act, PLR_LOG)) || fLogAll || cmd_table[cmd].log == LOG_ALWAYS) && cmd_table[cmd].log != LOG_NEVER)
        {
...
bool wiz_do(CHAR_DATA *ch, char *command)
{
        ...
        for (cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++)
        {
                if (command[0] == cmd_table[cmd].name[0] && !str_prefix(command, cmd_table[cmd].name) && (cmd_table[cmd].level <= get_trust(get_char(ch))) && cmd_table[cmd].level >= L_HER)
                {
                        return TRUE;
                }
        }
