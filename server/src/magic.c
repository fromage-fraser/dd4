...
void say_spell(CHAR_DATA *ch, int sn)
{
        ...
        if (!IS_SET(ch->act, ACT_WIZINVIS_MOB) && CAN_SPEAK(ch))
        {
...
void do_cast(CHAR_DATA *ch, char *argument)
{
        ...
        */
        ...
        if (((str_cmp(skill_table[sn].name, "abyssal hand")) == 0) && ch->race == RACE_TIEFLING)
        {
                mana = 25;
        }
        ...
        case TAR_CHAR_DEFENSIVE:
                ...
                else
                {
                        ...
                        if (!IS_NPC(victim) && !IS_NPC(ch) && victim->level > 10 && (abs(ch->level - victim->level) > 10) && (ch->level < LEVEL_HERO + 1))
                        {
        ...
        if (!IS_NPC(ch) && ch->level <= LEVEL_HERO && number_percent() > (65 + (ch->pcdata->learned[sn] / 3)) && ((sn = skill_lookup(arg1)) >= 0) && ch->pcdata->learned[sn] > 0)
        {
        ...
        else
        {
                ...
                if (ch->level >= LEVEL_IMMORTAL && (ch->mana < 1))
                {
                        ch->mana = 1;
                }
                ...
                {
                        if (victim->position == POS_DEAD || ch->in_room != victim->in_room || ch->position < POS_FIGHTING)
                                return;
        ...
        if (skill_table[sn].target == TAR_CHAR_OFFENSIVE && victim->master != ch && victim != ch && IS_AWAKE(victim) && !victim->fighting)
        {
                multi_hit(victim, ch, TYPE_UNDEFINED);
        }
}
...
void obj_cast_spell(int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj)
{
        ...
        case TAR_CHAR_DEFENSIVE:
                ...
                if (!IS_NPC(ch) && !IS_NPC(victim) && victim->level > 10 && abs(ch->level - victim->level) > 10 && ch->level < LEVEL_HERO + 1)
                {
...
void spell_call_lightning(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        for (vch = char_list; vch; vch = vch->next)
        {
                ...
                if (vch->in_room == ch->in_room && vch != ch && (IS_NPC(ch) ? !IS_NPC(vch) : IS_NPC(vch)))
                {
                        dam = dice(level, 8);
...
void spell_cure_blindness(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (!IS_AFFECTED(victim, AFF_BLIND) && (victim == ch))
        {
                return;
        }
...
void spell_earthquake(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        for (vch = char_list; vch; vch = vch->next)
        {
                ...
                {
                        if (vch == ch->mount || is_group_members_mount(vch, ch) || is_same_group(vch, ch))
                                continue;
...
void spell_giant_strength(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (is_affected(victim, gsn_giant_strength) || is_affected(victim, gsn_enhanced_strength))
                return;
...
void spell_knock(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((door = find_door(ch, arg)) >= 0)
        {
                ...
                if (!IS_SET(pexit->exit_info, EX_CLOSED) || !IS_SET(pexit->exit_info, EX_LOCKED) || IS_SET(pexit->exit_info, EX_BASHPROOF))
                {
...
void spell_identify(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        case ITEM_PIPE:
                ...
                else if ((obj->value[0] == obj->value[1]) || (((obj->value[0] * 100) / obj->value[1]) >= 99))
                {
        ...
        /* Send GMCP updates to web client so identified status appears immediately */
        if (ch)
        {
...
void spell_know_alignment(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (ap > 700)
                ...
                msg = "$N shows no remorse for $S actions, regardless of whom $E hurts.";
        else if (ap > -700)
                msg = "$N shows no remorse for $S actions, regardless of whom $E hurts.";
        else
...
void spell_locate_object(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        {
                if (!multi_keyword_match(arg2, obj->name) || !can_see_obj(ch, obj) || obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_REMAINS || IS_SET(obj->extra_flags, ITEM_BODY_PART))
                        continue;
...
void spell_summon_familiar(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (((ch->in_room->sector_type == SECT_UNDERWATER) || (ch->in_room->sector_type == SECT_UNDERWATER_GROUND)) && ((ch->race != RACE_SAHUAGIN) && (ch->race != RACE_GRUNG) && (ch->form != FORM_SNAKE)))
        {
...
void spell_summon_demon(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        for (mob = char_list; mob; mob = mob->next)
        {
                ...
                if (IS_NPC(mob) && mob->master == ch && (mob->pIndexData->vnum == MOB_VNUM_LESSER || mob->pIndexData->vnum == MOB_VNUM_DEMON))
                        count++;
        }
        ...
        if (ch->class == CLASS_SHAPE_SHIFTER)
        {
                ...
                if (mob->level > ch->level)
                        mob->level = ch->level;
                mob->max_hit = (in_sc_room) ? ((mob->level * mod_room_bonus) / 100) * 10 + number_range(mob->level * mob->level / 4, (mob->level * ((mob->level * mod_room_bonus) / 100))) : mob->level * 10 + number_range(mob->level * mob->level / 4, mob->level * mob->level);
                mob->hit = mob->max_hit;
        }
        else
        {
                ...
                if (mob->level > ch->level)
                        mob->level = ch->level;
                mob->max_hit = (in_sc_room) ? ((mob->level * mod_room_bonus) / 100) * 8 + number_range(mob->level * mob->level / 4, (mob->level * ((mob->level * mod_room_bonus) / 100))) : mob->level * 8 + number_range(mob->level * mob->level / 4, mob->level * mob->level);
                mob->hit = mob->max_hit;
        }
...
void spell_summon_avatar(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        for (rch = char_list; rch; rch = rch->next)
        {
                ...
                if (IS_NPC(rch) && rch->master == ch && rch->pIndexData->vnum == MOB_VNUM_WOLF)
                        count++;
        }
...
void spell_teleport(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        for (;;)
        {
                ...
                {
                        if (pRoomIndex && !IS_SET(pRoomIndex->area->area_flags, AREA_FLAG_NO_TELEPORT) && !IS_SET(pRoomIndex->room_flags, ROOM_PLAYER_KILLER) && !IS_SET(pRoomIndex->room_flags, ROOM_NO_RECALL) && !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) && !IS_SET(pRoomIndex->room_flags, ROOM_CONE_OF_SILENCE))
                                break;
                }
                else
                {
                        if (pRoomIndex && !IS_SET(pRoomIndex->room_flags, ROOM_NO_MOUNT) && !IS_SET(pRoomIndex->room_flags, ROOM_PRIVATE) && !IS_SET(pRoomIndex->area->area_flags, AREA_FLAG_NO_TELEPORT) && !IS_SET(pRoomIndex->room_flags, ROOM_PLAYER_KILLER) && !IS_SET(pRoomIndex->room_flags, ROOM_NO_RECALL) && !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) && !IS_SET(pRoomIndex->room_flags, ROOM_CONE_OF_SILENCE))
                                break;
                }
        }
        ...
        if (number_percent() < 2 * level && !saves_spell(level, victim) && !IS_SET(victim->in_room->room_flags, ROOM_PLAYER_KILLER) && !is_affected(victim, gsn_dragon_shield) && !is_affected(victim, gsn_bonus_exotic))
        {
                for (obj_lose = victim->carrying; obj_lose; obj_lose = obj_next)
                {
                        ...
                        default:
                                continue;
                        case ITEM_CONTAINER:
                                msg = "{Y$p ignites and burns!{x";
                                break;
                        case ITEM_POTION:
                                msg = "{Y$p bubbles and boils!{x";
                                break;
                        case ITEM_SCROLL:
                                msg = "{Y$p crackles and burns!{x";
                                break;
                        case ITEM_STAFF:
                                msg = "{Y$p smokes and chars!{x";
                                break;
                        case ITEM_WAND:
                                msg = "{Y$p sparks and sputters!{x";
                                break;
                        case ITEM_FOOD:
                                msg = "{Y$p blackens and crisps!{x";
                                break;
                        case ITEM_PILL:
                                msg = "{Y$p melts and drips!{x";
                                break;
                        case ITEM_PIPE:
                                msg = "{Y$p chars and blackens!{x";
                                break;
                        }
        ...
        if (number_percent() < 2 * level && !saves_spell(level, victim) && !IS_SET(victim->in_room->room_flags, ROOM_PLAYER_KILLER) && !is_affected(victim, gsn_dragon_shield) && !is_affected(victim, gsn_bonus_exotic))
        {
                for (obj_lose = victim->carrying; obj_lose; obj_lose = obj_next)
                {
                        ...
                        default:
                                continue;
                        case ITEM_SCROLL:
                                msg = "{Y$p is soggy and illegible!{x";
                                break;
                        case ITEM_FOOD:
                                msg = "{Y$p is saturated and slushy!{x";
                                break;
                        case ITEM_POISON_POWDER:
                                msg = "{Y$p turns into a watery paste!{x";
                                break;
                        case ITEM_PILL:
                                msg = "{Y$p is sodden and mushy!{x";
                                break;
                        case ITEM_CLIMBING_EQ:
                                msg = "{Y$p is soused and frays!{x";
                                break;
                        case ITEM_PAINT:
                                msg = "{Y$p is drenched and diluted!{x";
                                break;
                        case ITEM_AUCTION_TICKET:
                                msg = "{Y$p is soaked right through!{x";
                                break;
                        case ITEM_SMOKEABLE:
                                msg = "{Y$p becomes waterlogged pulp!{x";
                                break;
                        case ITEM_PIPE_CLEANER:
                                msg = "{Y$p is drenched and dripping!{x";
                                break;
                        }
...
void spell_share_strength(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((get_curr_str(ch) <= 5) || (IS_AFFECTED(ch, AFF_TORSO_TRAUMA)))
        {
...
void spell_moonray(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (weather_info.moonlight == MOON_WAXING_CRESCENT || weather_info.moonlight == MOON_WANING_CRESCENT)
                dam *= 1.1;

        if (weather_info.moonlight == MOON_FIRST_QUARTER || weather_info.moonlight == MOON_LAST_QUARTER)
                dam *= 1.2;

        if (weather_info.moonlight == MOON_WAXING_GIBBOUS || weather_info.moonlight == MOON_WANING_GIBBOUS)
                dam *= 1.3;
...
void spell_recharge_item(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (obj->value[2] > obj->value[1] && in_sc_room == FALSE)
        {
                act("It then shudders violently and implodes!", ch, obj, NULL, TO_CHAR);
                extract_obj(obj);
        }

        if (obj->value[2] > obj->value[1] && in_sc_room == TRUE)
        {
...
void spell_lore(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (obj->item_type != ITEM_ARMOR && obj->item_type != ITEM_WEAPON && obj->item_type != ITEM_LIGHT && obj->item_type != ITEM_TREASURE && obj->item_type != ITEM_TRASH && obj->item_type != ITEM_CONTAINER && obj->item_type != ITEM_FURNITURE && obj->item_type != ITEM_DRINK_CON && obj->item_type != ITEM_KEY && obj->item_type != ITEM_FOOD && obj->item_type != ITEM_MONEY && obj->item_type != ITEM_BOAT && obj->item_type != ITEM_CORPSE_PC && obj->item_type != ITEM_CORPSE_NPC && obj->item_type != ITEM_FOUNTAIN && obj->item_type != ITEM_CLIMBING_EQ && obj->item_type != ITEM_ANVIL && obj->item_type != ITEM_MOB && obj->item_type != ITEM_AUCTION_TICKET && obj->item_type != ITEM_POISON_POWDER && obj->item_type != ITEM_LOCK_PICK && obj->item_type != ITEM_INSTRUMENT && obj->item_type != ITEM_ARMOURERS_HAMMER && obj->item_type != ITEM_MITHRIL && obj->item_type != ITEM_DIGGER && obj->item_type != ITEM_WHETSTONE && obj->item_type != ITEM_CRAFT && obj->item_type != ITEM_TURRET_MODULE && obj->item_type != ITEM_TURRET && obj->item_type != ITEM_FORGE && obj->item_type != ITEM_PIPE && obj->item_type != ITEM_PIPE_CLEANER && obj->item_type != ITEM_REMAINS)
        {
...
void spell_animate_weapon(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((obj = get_eq_char(victim, WEAR_DUAL)) && !IS_SET(obj->extra_flags, ITEM_BODY_PART))
        {
...
void spell_transport(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((!(victim = get_char_world(ch, target_name))) || (!IS_NPC(ch) && ch->fighting) || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY) || victim->level >= level + 3 || victim == ch)
        {
        ...
        if (victim->carry_number + get_obj_number(obj) > can_carry_n(victim) || victim->carry_weight + victim->coin_weight + get_obj_weight(obj) > can_carry_w(victim))
        {
...
void spell_mass_teleport(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        {
                pRoomIndex = get_room_index(number_range(0, 65535));
                if (pRoomIndex && !IS_SET(pRoomIndex->room_flags, ROOM_PRIVATE) && !IS_SET(pRoomIndex->room_flags, ROOM_NO_RECALL) && !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) && pRoomIndex->area == ch->in_room->area)
                        break;
        }
...
bool mob_interacts_players(CHAR_DATA *mob)
{
        ...
        if (mob->rider && (IS_SET(mob->in_room->room_flags, ROOM_SAFE) || IS_SET(mob->in_room->area->area_flags, AREA_FLAG_SAFE)))
                return FALSE;
...
void spell_psychometry(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((obj->item_type != ITEM_KEY) || ((obj->item_type = ITEM_KEY) && (obj->value[0] == 0)))
        {
...
void spell_flood(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if ((pexit = ch->in_room->exit[5]) && (!IS_SET(pexit->exit_info, EX_ISDOOR)))
        {
...
void spell_fleshrot(int sn, int level, CHAR_DATA *ch, void *vo)
{
        ...
        if (IS_NPC(victim) && (IS_SET(victim->act, ACT_OBJECT) || IS_INORGANIC(victim)))
        {
        ...
        if ((is_affected(victim, gsn_poison)) || (is_affected(victim, gsn_nausea)) || (is_affected(victim, sn)) || (is_affected(victim, gsn_prayer_plague)))
        {
