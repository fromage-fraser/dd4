...
/* existing from your sound system */
...
/* Queue a keyed mob death cry sound to everyone in a room. */
void sound_mobdeath_room_key( ROOM_INDEX_DATA *room, const char *key, int base_vol )
{
        ...
        if ( !key || !*key )
                key = "sfx.combat.mobdeath.1";

        ev = sound_event_lookup( key );
        if ( !ev )
                return;

        file = ( ev->files[0] && *ev->files[0] ) ? ev->files[0] : NULL;
        if ( !file )
                return;
        ...
        for ( vch = room->people; vch; vch = vch->next_in_room )
        {
                ...
                /* Use event default volume if present, else caller's base_vol. */
                vol_use = ( ev->default_volume > 0 ) ? ev->default_volume : base_vol;
                vol_adj = media_apply_volume( vol_use, vch, "sfx", "sfx" );
                if ( vol_adj <= 0 )
                        continue;

                /* Hard priority: flush anything already queued for this client. */
                /* sfx_queue_clear( vch->desc ); */

                /* Combat: no extra delay. */
...
/* Queue a generic mob death cry sound to everyone in a room. */
void sound_mobdeath_room( ROOM_INDEX_DATA *room, int base_vol )
{
        sound_mobdeath_room_key( room, "sfx.combat.mobdeath.1", base_vol );
}
...
void sfx_queue_clear( DESCRIPTOR_DATA *d )
{
        ...
        /* Optional, but keeps logs/debugging clean. */
        memset( d->sfx_q, 0, sizeof( d->sfx_q ) );

        if (SND_LOG_ENABLED ) { log_stringf( "SFXQ: CLEAR d=%p", d ); }
}

static const char *sound_event_pick_file( const sound_event_def *ev )
{
        int i, n = 0, total = 0, roll;
        ...
        {
                if ( ev->files[i] && *ev->files[i] )
                        total += ( ev->weights[i] > 0 ) ? ev->weights[i] : 0;
        }

        if ( total <= 0 )
        {
                int pick = number_range( 0, n - 1 );
                for ( i = 0; i < 32; i++ )
                {
                        if ( ev->files[i] && *ev->files[i] )
                        {
...
static bool is_target_inorganic( CHAR_DATA *victim )
{
        if ( !victim )
                return FALSE;

        if ( !IS_NPC(victim) )
                return FALSE; /* players organic */
...
void sound_combat_hit_sfx( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
        ...
        vol = ev->default_volume;

        for ( vch = ch->in_room ? ch->in_room->people : NULL; vch; vch = vch->next_in_room )
        {
                int vol_adj;
                ...
                vol_adj = media_apply_volume( vol, vch, "sfx", "sfx" );
                if ( vol_adj <= 0 )
                        continue;
...
void sound_combat_parry_sfx( CHAR_DATA *ch, CHAR_DATA *victim )
{
        ...
        if ( wield )
                key = "sfx.combat.parry.weapon";
        else
                key = "sfx.combat.parry.noweapon";
...
static void sound_make_spell_key( char *buf, int size, const char *phase, int sn )
{
        ...
        {
                snprintf( buf, size, "sfx.spell.%s.generic", phase );
                return;
        }

        for ( i = 0, j = 0; skill_table[sn].name[i] && j < (int)sizeof(name) - 1; i++ )
        {
                if ( skill_table[sn].name[i] == ' ' )
                        name[j++] = '_';
                else
                        name[j++] = LOWER( skill_table[sn].name[i] );
        }

        name[j] = '\0';

        snprintf( buf, size, "sfx.spell.%s.%s", phase, name );
}
...
void sound_spell_sfx( CHAR_DATA *ch, int sn, const char *phase )
{
        ...
        if ( !ch || !ch->in_room || !phase || !*phase )
                return;

        sound_make_spell_key( key, sizeof(key), phase, sn );

        if ( SND_LOG_ENABLED )
        {
                log_stringf( "SpellSFX: trying key=%s sn=%d phase=%s",
                             key, sn, phase );
        }
        ...
        if ( !ev )
        {
                snprintf( key, sizeof(key), "sfx.spell.%s.generic", phase );
                ev = sound_event_lookup( key );
        }
...
void update_weather_for_char( CHAR_DATA *ch )
{
      ...
      || ( ch->in_room->sector_type == SECT_UNDERWATER_GROUND ) )
    {
        if (p->MediaWeatherActive)
        {
            ...
            p->MediaWeatherActive = FALSE;
             ...
             if ( SND_LOG_ENABLED) { log_stringf("WeatherSFX: stop (indoors) for %s", ch->name); }
        }
        return;
    }
    ...
    if (!ev || !ev->files[0] || !*ev->files[0])
    {
        if (p->MediaWeatherActive)
        {
            ...
            if ( SND_LOG_ENABLED) { log_stringf("WeatherSFX: stop (clear sky) for %s", ch->name); }
        }
        return;
    }
        ...
        || p->MediaWeatherVol != vol)
    {
        ...
        if (p->MediaWeatherActive)
        {
            ...
            if ( SND_LOG_ENABLED) { log_stringf("WeatherSFX: stop previous (switch) for %s", ch->name); }
        }
        ...
        if ( SND_LOG_ENABLED) { log_stringf("WeatherSFX: start %s vol=%d for %s",
                    ev->key, vol, ch->name); }
    }
                  ...
                  int delay_ticks )
{
        ...
        if ( next == d->sfx_head )
        {
                if (SND_LOG_ENABLED ) { log_stringf( "SFXQ: DROP d=%p head=%d tail=%d cd=%d tag=%s vol=%d file=%s",
                             d,
                             d->sfx_head,
                             d->sfx_tail,
                             d->sfx_cooldown,
        ...
        e->delay_ticks = delay_ticks;

        if ( SND_LOG_ENABLED ) { log_stringf( "SFXQ: ENQ d=%p head=%d tail=%d next=%d cd=%d delay=%d tag=%s vol=%d id=%s file=%s",
                     d,
                     d->sfx_head,
                     d->sfx_tail,
                     next,
...
void sound_sfx_update( void )
{
        ...
        for ( d = descriptor_list; d; d = d->next )
        {
                ...
                if (SND_LOG_ENABLED ) { log_stringf( "SFXQ: DEQ d=%p head=%d tail=%d cd=%d tag=%s vol=%d id=%s file=%s",
                    d,
                    d->sfx_head,
                    d->sfx_tail,
                    d->sfx_cooldown,
                ...
                if ( SND_LOG_ENABLED ) { log_stringf("SFXQ: TICK d=%p cd=%d head=%d tail=%d", d, d->sfx_cooldown, d->sfx_head, d->sfx_tail); }

                d->sfx_cooldown = (e->delay_ticks > 0) ? e->delay_ticks : 0;

                if ( SND_LOG_ENABLED ) { log_stringf( "SFXQ: POST d=%p new_head=%d tail=%d set_cd=%d",
                    d,
                    d->sfx_head,
                    d->sfx_tail,
                    d->sfx_cooldown ); }
    ...
    if (arg1[0] == '+' || arg1[0] == '-')
    {
        ...
        else if (!str_cmp(arg1+1, "ambient"))
        {
            if (set)
            {
                ...
                if ( SND_LOG_ENABLED) { log_stringf("WeatherSFX: reassert after sconfig for %s", ch->name); }
            }
            else
            {
                ch->pcdata->snd_env = 0;
                ...
                if (ch->desc && ch->desc->pProtocol && ch->desc->pProtocol->bGMCP)
                {
                    ...
                    GMCP_Media_Stop(ch->desc, "\"key\":\"dd.ambient.room\",\"type\":\"ambient\",\"fadeaway\":true,\"fadeout\":700");
                    GMCP_Media_Stop(ch->desc, "\"key\":\"dd.ambient.area\",\"type\":\"ambient\",\"fadeaway\":true,\"fadeout\":700");
                    GMCP_Media_Stop(ch->desc, "\"key\":\"dd.ambient.sector.A\",\"type\":\"ambient\",\"fadeaway\":true,\"fadeout\":700");
                    GMCP_Media_Stop(ch->desc, "\"key\":\"dd.ambient.sector.B\",\"type\":\"ambient\",\"fadeaway\":true,\"fadeout\":700");
                    GMCP_Media_Stop(ch->desc, "\"key\":\"dd.ambient.weather\",\"type\":\"ambient\",\"fadeaway\":true,\"fadeout\":700");
                    media_env_refresh(ch, ch->in_room, TRUE);
        ...
        else if (!str_cmp(arg1+1, "sfx"))
        {
            if (set)
            {
                ...
                if (ch->pcdata->snd_sfx == 0)
                    ch->pcdata->snd_sfx = SND_DEF_SFX;
                send_to_char("Sfx are now {GON{x.\n\r", ch);
            }
            else
            {
                ch->pcdata->snd_sfx = 0;
                send_to_char("Sfx are now {ROFF{x.\n\r", ch);
            }
            return;
        }
